...
export default function TopStoreShowAllComponent({ item, topstoreDetails }) {
  ...
  return (
    <Paper elevation={3} style={{ borderRadius: 10 }}>
        ...
        <Grid2 style={{ marginLeft: matches?5:20 }}>
          ...
          >
            <div style={{ fontSize: 27, fontWeight: "bold", color: "#333" }}>
              {item.couponname.length < 40
                ? item.couponname
                : item.couponname.slice(0, 40) + "..."}
            </div>
            <div
              style={{
                fontSize: 14,
      ...
      >
        <Accordion style={{ color: "#B53471", padding:matches?"4px 4px 4px 5px ": "4px 4px 4px 25px " }}>
